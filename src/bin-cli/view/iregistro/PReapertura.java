...
public class PReapertura extends JPanel {
    ...
    public JLabel labTRegistro, labPerito, labFechaAper, fechaApert, labEstado, estadoReg, labCompania, labRamo,
            ...
            labEstPago, labFCierre, labFFacPerito, labFAno, labFMes, labObserv, labUsuario, labTipoInt, labTipoReap;
    public JComboBox boxPerito, boxCompania, boxRamo, boxTipoSin, boxEstadoPago, boxTipoInt, boxTipoReap;
    public JTextField textRefCia, textNPoliza, textRContacto, textAsegurado, textDirCont,
            textDirRies, textSContacto, textNFactura, textFMes, textFAno, textGHonor, textGLocom, textGVarios, textGSuma, textGIva,
    ...
    public JButton verificar, cerrarReg, guardar, reaperturarReg, abrirReapert, entregaPerito, anadirTelefono, modifTelfono, elimTelefono;
     ...
     */
    public PReapertura(Registro asociado, Reapertura reapertura, int codVent){
        rp = reapertura;
        registro = asociado;
        ...
        grid.insets = new Insets(0,0,0,8);
        grid.gridwidth = 4;
        grid.weighty = 0.6;
        grid.gridheight = 3;
        ...
        grid = newPosicion(GridBagConstraints.WEST,
                GridBagConstraints.HORIZONTAL,4,18);
        ...
        add(labTipoReap, grid);

        grid = newPosicion(GridBagConstraints.NORTH,
                GridBagConstraints.HORIZONTAL,4,6);
        add(boxTipoReap, grid);
    ...
    public void initComponents() {
        ...
        labTReaperturas = new JLabel("Registros Asociados:");
        labTipoReap = new JLabel("Tipo reapertura:");
        ...
        boxTipoReap = new JComboBox();
        boxTipoReap.setBackground(Color.WHITE);
        ...
        textPAjustes.addFocusListener(new FocusListener() {
        ...
        areaDescrip.addFocusListener(new FocusListener() {

            @Override
            public void focusGained(FocusEvent e) {
                if (Config.configuracion.isCheckAmpliacion()){
                    ampliacion = new AmpliacionCampos(idInterfaz, registro);
                }
            }
        ...
        guardar.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                Reapertura rt = getReaperturaFrame();

                if (registro.getEstadoRegistro().equals("NUEVO")) {

                    if (!boxTipoReap.getSelectedItem().equals("Default")) {
                                            ...
                                            if (!boxTipoSin.getSelectedItem().equals("Default")) {
                                                if (!boxTipoInt.getSelectedItem().equals("Default")) {
                                                    ...
                                                    for (TipoReapertura rtt : DataTables.getTipoReaperturas()) {
                                                        if (rtt.getTipoReapertura().equals(boxTipoReap.getSelectedItem())) {
                                                    ...
                                                    rp.setTipoReapertura(rt.getTipoReapertura());
                                                    packet.setReapertura(rp);
                    ...
                    if (compareRegistro(t) && compareReapertura(rt)) Config.warning("No se detectan modificaciones en el Registro.");
                    else{
                                            ...
                                            if (!boxRamo.getSelectedItem().equals("Default")) {
                                                if (!boxTipoSin.getSelectedItem().equals("Default")) {
                                                    if (!boxTipoInt.getSelectedItem().equals("Default")) {
                                                        ...
                                                        for (TipoReapertura rtt : DataTables.getTipoReaperturas()) {
                                                            if (rtt.getTipoReapertura().equals(boxTipoReap.getSelectedItem())) {
                                                                rp.setTipoReapertura(rtt.getCodTipoReapertura());
                                                                break;
                                                            }
                                                        }
                                                        SafePacket packet = new SafePacket(6, Config.getUser(), Config.getIpClient());
                                                        packet.setRegistro(t);
                                                        System.out.println("id ventana al enviar"+getIdInterfaz());
                ...
                if (registro.getTipoRegistro().equals("REGISTRO")) {
                    ...
                    if (registro.getEstadoRegistro().equals("NUEVO")) {
                        ...
                        if (verificarTelf(numeroNuevo) && duplicatedTelf(numeroNuevo)) {
                            ...
                            int regAsocAseg = 0;
                            for (Registro r : DataTables.getRegistros()) {
                                if (r.getCodRegistro() == rp.getRegistroAsociado()){
                                    regAsocAseg = r.getSiniestro().getAsegurado().getCodAsegurado();
                                }
                            }
                            TelefonoAsegurado tel = new TelefonoAsegurado(0, nuevoTelefono, regAsocAseg);
                            TelfPacket packet = new TelfPacket(1, Config.getUser(), Config.getIpClient());
                            packet.setRegistro(registro);
                            packet.setCodVentana(getIdInterfaz());
                            packet.setTelefonoAsegurado(tel);
        ...
        //BOTON ABRIR REAPERTURA
        abrirReapert = new JButton("Abrir Registro");
        abrirReapert.setBackground(Config.AUX5);
                    ...
                    public void run() {
                        try{
                            ...
                            if (!isOpened){
                                ...
                                if (regi.getTipoRegistro().equals("REGISTRO")){
                                    InterfazRegistro inter = new InterfazRegistro(regi);
                            ...
                            */
                            if (InterfazRegistro.existsInterfaceReg(regi.getCodRegistro())) {
                                if (regi.getTipoRegistro().equals("REGISTRO")){
                                    InterfazRegistro inter = new InterfazRegistro(regi);
                                    //Config.interfaces.add(inter);
                                    InterfazRegistro.addInterface(inter);
                                } else if (regi.getTipoRegistro().equals("REAPERTURA")) {
                                    Reapertura temp = new Reapertura(0);
                                    temp = temp.getReaperturaFromCodigo(codReap);
                                    InterfazRegistro inter = new InterfazRegistro(regi, temp);
                                    //Config.interfaces.add(inter);
    ...
    public void chargeCamposBox(){
        ...
        if (registro.getEstadoRegistro().equals("CERRADO")){
            ...
            for (TipoReapertura tr : DataTables.getTipoReaperturas()) boxTipoReap.addItem(tr.getTipoReapertura());
            ...
            for (TipoReapertura tr : DataTables.getTipoReaperturas()) if(tr.getEstadoTipoReapertura().equals("ACTIVO")) boxTipoReap.addItem(tr.getTipoReapertura());
    ...
    /*
     * Metodo que carga el devuelve una Reapertura de los campos del Frame
     */
    public Reapertura getReaperturaFrame(){
        Reapertura temp = new Reapertura(rp.getCodReapertura(), rp.getRegistroAsociado(), rp.getReapertura(), rp.getTipoReapertura());
        for (TipoReapertura r : DataTables.getTipoReaperturas()){
            if (r.getTipoReapertura().equals(boxTipoReap.getSelectedItem())){
                temp.setTipoReapertura(r.getCodTipoReapertura());
                break;
            }
        }
        return temp;
    ...
    /*
     * Metodo que compara el campo Tipo Reapertura de dos reaperturas
     */
    public boolean compareReapertura(Reapertura target){
        boolean igual = true;
        if (target.getTipoReapertura() != rp.getTipoReapertura()) igual = false;
        return igual;
    }
    ...
    public void chargeOpenRegistro(){
        ...
        labTRegistro.setText("REAPERTURA -- CÓDIGO "+codReg+"-"+anoReg);

        //Seleccion del perito
        for (Perito per : DataTables.getPeritos()){
            if (per.getCodPerito() == registro.getPerito()) boxPerito.setSelectedItem(per.getNombrePerito());
        }
        ...
        for(TipoReapertura tr : DataTables.getTipoReaperturas()){
            if (rp.getTipoReapertura() == tr.getCodTipoReapertura()){
                boxTipoReap.setSelectedItem(tr.getTipoReapertura());
                break;
            }
        }
        ...
        ArrayList<TelefonoAsegurado> telfA = chargeTelefonosBase(rp.getRegistroAsociado());
    ...
    public void chargeReaperturasReg(){
        ...
        int regAsoc = rp.getRegistroAsociado();
        for (int i = 0; i < filas; i++){
            modelReapert.removeRow(0);
        }
        ArrayList<Reapertura> reaperturas = new ArrayList<>();
        for (Reapertura r : DataTables.getReaperturas()){
            if (r.getRegistroAsociado() == regAsoc){
                reaperturas.add(r);
            }
        }
        for (Registro regi : DataTables.getRegistros()){
            if (regi.getCodRegistro() == regAsoc){
                Object [] tb = {rp.getRegistroAsociado(), new SimpleDateFormat("dd/MM/yyyy").format(regi.getFechaApertura())+" (Base)"};
                modelReapert.addRow(tb);
                break;
            }
        }
    ...
    public ArrayList<TelefonoAsegurado> chargeTelefonosBase(int regBase){
        ArrayList<TelefonoAsegurado> lista = new ArrayList<TelefonoAsegurado>();
        for(Registro r : DataTables.getRegistros()){
            if (r.getCodRegistro() == regBase){
                lista = r.getSiniestro().getAsegurado().getTelefonosAseg();
            }
        }
        return lista;
    }
    ...
    public void validarCierreRegistro(){
        ...
        //Valida estado de pago del Registro
        String status = (String) boxEstadoPago.getSelectedItem();
        boolean isValidPago = false;
        if (status.equals("PAGO")) isValidPago = true;
        else {
            int opt = Config.option("El Pago del Registro figura como IMPAGO. ¿Quiere cerrarlo de todas formas?");
            if (opt == 0) isValidPago = true;
        }
        ...
        if (isValidPago && isValidEntrega) {
            try{
                ...
                int opt = Config.option("La fecha de cierre no ha sido introducida," +
                        "\n ¿Quiere autorrellenar el campo con la fecha de hoy?");
                if (opt == 0) {
                    registro.setFechaCierre(new Date());
                    registro.setUsuario(Config.getUser().getCodUsuario());
                    registro.setEstadoRegistro("CERRADO");
    ...
    public void cerrarRegistro(){
        ...
        safePacket.setRegistro(registro);
        safePacket.setCodVentana(idInterfaz);
    ...
    public void setReapertura(Reapertura rp) {
        this.rp = rp;
    }
